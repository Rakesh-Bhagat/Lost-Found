...
export async function GET() {
  try {
    ...
    return NextResponse.json({ message: "Error fetching message threads" }, { status: 500 })
  }
}
  ...
  try {
    ...
    const existingThread = await prisma.messageThread.findFirst({
      where: {
        ...
        OR: [
          {
            AND: [{ participant1Id: session.user.id }, { participant2Id: receiverId }],
          },
          {
            AND: [{ participant1Id: receiverId }, { participant2Id: session.user.id }],
          },
        ],
      },
    })
    ...
    return NextResponse.json({ message: "Error creating message thread" }, { status: 500 })
  }
}
